...
public class BellmanFord{
    ...
    BellmanFord(WGraph g, int source) throws BellmanFordException{
    	...
    	this.predecessors[this.source] = source;
    	
    	for(int i = 0; i<g.getNbNodes()-1; i++){
    		for(Edge e : g.getEdges()) {
    			if(distances[e.nodes[0]] != Integer.MAX_VALUE && distances[e.nodes[1]] > distances[e.nodes[0]]+ e.weight) {
    ...
    public int[] shortestPath(int destination) throws BellmanFordException{
    	...
    	//find array size
    	int theOneBefore = destination;
    	int size = 1;
    	while(theOneBefore!=this.source) {
    		theOneBefore = this.predecessors[theOneBefore];
    		size++;
    	}
    	//build the path array
    	theOneBefore = destination;
    	int[] path = new int[size];
    	for(int i =0; i<size; i++) {
    		path[size-1-i] = theOneBefore;
    		theOneBefore = this.predecessors[theOneBefore];
    	}
